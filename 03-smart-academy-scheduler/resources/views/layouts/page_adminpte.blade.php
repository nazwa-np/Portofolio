...
<head>
    ...
    <title>Prodi Pendidikan Teknik Elektronika</title>
    ...
    <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4" id="sidenav-main">
        <div class="sidenav-header">
          ...
          <a class="navbar-brand m-0" href="#">
            ...
            <span class="ms-1 font-weight-bold">Admin PTE</span>
          </a>
        </div>
        ...
        <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
            <ul class="navbar-nav">
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpte.index3') ? 'active' : '' }}"
                     href="{{ route('adminpte.index3') }}">
                    <i class="fas fa-database me-2"></i>
                    <span class="nav-link-text">Input Data</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpte.save_datapte') ? 'active' : '' }}"
                     href="{{ route('adminpte.save_datapte') }}">
                    <i class="fas fa-save me-2"></i>
                    <span class="nav-link-text">Data Tersimpan</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpte.edit') ? 'active' : '' }}"
                     href="{{ route('adminpte.edit', auth()->user()->id) }}">
                    <i class="fas fa-edit me-2"></i>
                    <span class="nav-link-text">Edit Data</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpte.insert') ? 'active' : '' }}"
                     href="{{ route('adminpte.insert') }}">
                ...
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('jadwal.index') ? 'active' : '' }}"
                    ...
                    <i class="fas fa-plus-circle me-2"></i>
                    <span class="nav-link-text">Jadwal Perkuliahan</span>
                  </a>
                </li>
          </ul>
        </div>
        ...
        <script>
            ...
            // Alert handling for session messages
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "{{ route('adminpte.save_datapte') }}";
                    }
                });
                @endif
