    ...
    <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4" id="sidenav-main">
        <div class="sidenav-header">
          ...
          <a class="navbar-brand m-0" href="#">
            ...
            <span class="ms-1 font-weight-bold">Admin Informatika</span>
          </a>
        </div>
        ...
        <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
            <ul class="navbar-nav">
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminti.index1') ? 'active' : '' }}"
                     href="{{ route('adminti.index1') }}">
                    {{-- Font Awesome icon --}}
                ...
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminti.save_datati') ? 'active' : '' }}"
                     href="{{ route('adminti.save_datati') }}">
                    <i class="fas fa-save me-2"></i>
                    <span class="nav-link-text">Data Tersimpan</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminti.edit') ? 'active' : '' }}"
                     href="{{ route('adminti.edit', auth()->user()->id) }}">
                    <i class="fas fa-edit me-2"></i>
                    <span class="nav-link-text">Edit Data</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminti.insert') ? 'active' : '' }}"
                     href="{{ route('adminti.insert') }}">
                    <i class="fas fa-plus-circle me-2"></i>
                    <span class="nav-link-text">Tambah Data</span>
                  </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('jadwal.adminti') }}" class="nav-link {{ request()->routeIs('jadwal.adminti') ? 'active' : '' }}">
        ...
        <script>
            ...
            // Alert handling for session messages
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "{{ route('adminti.save_datati') }}";
                    }
                });
                @endif
