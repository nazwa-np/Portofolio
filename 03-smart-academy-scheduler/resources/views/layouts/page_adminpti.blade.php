...
<head>
    ...
    <title>Prodi Pendidikan Teknik Informatika</title>
    <!-- DataTables CSS -->
    ...
    <style>
        ...
        #datatable-pti th:last-child,
        #datatable-pti td:last-child {
            border-right: none;
        }
        #datatable-ti {
    ...
    <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4" id="sidenav-main">
        <div class="sidenav-header">
          ...
          <a class="navbar-brand m-0" href="#">
            ...
            <span class="ms-1 font-weight-bold">Admin PTI</span>
          </a>
        </div>
        ...
        <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
            <ul class="navbar-nav">
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpti.index2') ? 'active' : '' }}"
                     href="{{ route('adminpti.index2') }}">
                    {{-- Font Awesome icon --}}
                ...
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpti.save_datapti') ? 'active' : '' }}"
                     href="{{ route('adminpti.save_datapti') }}">
                    <i class="fas fa-save me-2"></i>
                    <span class="nav-link-text">Data Tersimpan</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpti.edit') ? 'active' : '' }}"
                     href="{{ route('adminpti.edit', auth()->user()->id) }}">
                    <i class="fas fa-edit me-2"></i>
                    <span class="nav-link-text">Edit Data</span>
                  </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link {{ request()->routeIs('adminpti.insert') ? 'active' : '' }}"
                     href="{{ route('adminpti.insert') }}">
                    <i class="fas fa-plus-circle me-2"></i>
                    <span class="nav-link-text">Tambah Data</span>
                  </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('jadwal.adminpti') }}" class="nav-link {{ request()->routeIs('jadwal.adminpti') ? 'active' : '' }}">
        ...
        <script>
            ...
            // Alert handling for session messages
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "{{ route('adminpti.save_datapti') }}";
                    }
                });
                @endif
