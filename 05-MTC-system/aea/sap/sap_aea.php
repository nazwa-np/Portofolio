...
$title = "Dashboard";
require_once "../template_aea/header.php";
require_once "../template_aea/navbar.php";
require_once "../template_aea/sidebar.php";
?>
...
<div id="layoutSidenav_content">
    <main>
        <div class="container">
            <h2>Entry SAP</h2>
            <?php
            // Query data from list_aea with required columns
            $query_aea = "SELECT l.material_number, l.material_desc, l.unit, l.kuota_mps, COALESCE(m.reqmnt_qty, 0) as reqmnt_qty 
                           FROM list_aea l 
                           LEFT JOIN material m ON l.material_number = m.material_number AND m.cost_ctr = 'P1LP10'";
            $result_aea = $koneksi->query($query_aea);

            // Fetch data from list_aea
            if ($result_aea->num_rows > 0) {
                $data_aea = $result_aea->fetch_all(MYSQLI_ASSOC);
            } else {
                $data_aea = [];
            }
            ?>
                ...
                <form id="selected-form" method="post" action="selected_items.php">
                    <div class="table-wrapper">
                        <table>
                            ...
                            <tbody>
                            <?php 
                            // Iterate through data_aea to display rows
                            foreach ($data_aea as $index => $item_aea): 
                                // Initialize variables
                                $material_number = $item_aea['material_number'];
                                $kuota_mps = $item_aea['kuota_mps'];
                                $reqmnt_qty = $item_aea['reqmnt_qty'];
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="sticky-3"><?php echo $item_aea['material_desc']; ?></td>
                                    <td><?php echo $item_aea['unit']; ?></td>
